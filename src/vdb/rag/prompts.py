...
EVENT_RELEVANCE_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """Ты строгий классификатор релевантности событий.

Верни только "YES" или "NO":
- "YES" если событие тематически соответствует запросу пользователя
- "NO" если событие нерелевантно или дата не совпадает с явной датой в запросе

Правила:
- Сравнивай тематику запроса и события по названию, описанию, тегам
- Если в запросе про музыку/рок, но в событии нет явных музыкальных/рок-маркеров → "NO"
- Если событие про фото/фотографию/мастер-класс/джаз/классическую музыку/выставку/галерею/музей/театр/кафе/ресторан, а запрос про рок-музыку → "NO"
- Если в запросе указана дата и дата события не совпадает → "NO"
- Если не уверены в релевантности → "NO" (консервативно)
"""),
    ("human", """Запрос пользователя: {user_query}
...
Релевантно ли событие запросу? Ответь только YES или NO."""),
])
