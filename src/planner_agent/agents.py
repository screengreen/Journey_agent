...
КРИТИЧЕСКИ ВАЖНО:
- Список событий выше уже отфильтрован по релевантности запросу пользователя. Используй только эти события (не добавляй новые, не включай нерелевантные).
- Если событий мало (1-3), включи все. Если больше — включай максимально возможное количество, соблюдая ограничения.
- Если по какой-то причине план пуст, всё равно верни план хотя бы с одним событием из списка.

Для каждого события укажи ОБЯЗАТЕЛЬНО:
- Время начала (start_time) - ОБЯЗАТЕЛЬНО в формате "HH:MM", например "18:00", "09:30", БЕЗ даты! НЕ может быть None!
- Время окончания (end_time) - ОБЯЗАТЕЛЬНО в формате "HH:MM", например "20:00", "11:30", БЕЗ даты! НЕ может быть None!
- Продолжительность (duration_minutes) - ОБЯЗАТЕЛЬНО в минутах (целое число), например 120 для 2 часов
- Режим транспорта (transport_mode) - ОБЯЗАТЕЛЬНО: "walking", "car" или "bus" - ВЫБИРАЙ ОПТИМАЛЬНЫЙ на основе времени в пути:
  * walking - только если время в пути < 10 минут
  * bus или car - если время в пути пешком > 10 минут (выбирай самый быстрый вариант)
- Время в пути до этого события (travel_time_minutes) - в минутах (может быть None для первого события) - ИСПОЛЬЗУЙ ТОЧНЫЕ ДАННЫЕ ИЗ МАРШРУТОВ для выбранного транспорта!
...
КРИТИЧЕСКИ ВАЖНО: 
1. Даже если тебе не хватает данных (например, отсутствует информация о маршрутах, времени в пути или других деталях), ОБЯЗАТЕЛЬНО все равно составь итоговый план похода на основе доступной информации, с учетом всех имеющихся ограничений и событий.
2. Если чего-то не хватает, используй инструменты для получения информации, но ВСЕГДА выдай итоговый детальный план с хотя бы одним событием из списка.
3. План НЕ должен быть пустым - включи хотя бы одно событие из доступных!
4. ВАЖНО: Для КАЖДОГО события в плане ОБЯЗАТЕЛЬНО укажи:
   - start_time (время начала) - НЕ может быть None, формат "HH:MM" (например, "18:00")
   - end_time (время окончания) - НЕ может быть None, формат "HH:MM" (например, "20:00")
   - duration_minutes (продолжительность) - НЕ может быть None, целое число в минутах (например, 120 для 2 часов)
   - transport_mode (режим транспорта) - НЕ может быть None, одно из: "walking", "car", "bus"
   - event_name (название события) - НЕ может быть пустым
   - event_address (адрес события) - НЕ может быть пустым
   
Если в данных события нет точного времени, используй разумные оценки на основе описания события или стандартные временные интервалы (например, концерты обычно 2-3 часа, выставки 1-2 часа)."""
        ...
        # Если событий мало (≤3), строим детерминированный простой план без LLM
        if len(events) <= 3:
            from datetime import datetime, timedelta, time
            from src.planner_agent.models import Plan as PlanModel, PlanItem

            def _parse_time_from_date(dt_str: Optional[str]) -> Optional[time]:
                if not dt_str:
                    return None
                # Пытаемся выцепить HH:MM
                try:
                    if "T" in dt_str:
                        return datetime.fromisoformat(dt_str).time().replace(second=0, microsecond=0)
                    if " " in dt_str:
                        return datetime.strptime(dt_str, "%Y-%m-%d %H:%M").time()
                    # форматы "HH:MM"
                    return datetime.strptime(dt_str[-5:], "%H:%M").time()
                except Exception:
                    return None
            ...
            for ev in events:
                start_t = _parse_time_from_date(ev.date) or current_time
                end_dt = (datetime.combine(datetime.today(), start_t) + timedelta(hours=2))
                end_t = end_dt.time().replace(second=0, microsecond=0)
                duration = int((datetime.combine(datetime.today(), end_t) - datetime.combine(datetime.today(), start_t)).total_seconds() // 60)
                items.append(
                    PlanItem(
                        event_name=ev.title or "Событие",
                        event_address=ev.location or "Адрес не указан",
                        start_time=start_t,
                        end_time=end_t,
                        duration_minutes=duration,
                        transport_mode="walking",
                        travel_time_minutes=None,
                        notes=ev.description[:120] if ev.description else ""
                    )
                )
                # Сдвигаем текущее время после конца события + 30 мин
                current_time = (datetime.combine(datetime.today(), end_t) + timedelta(minutes=30)).time().replace(second=0, microsecond=0)

            total_duration = sum(it.duration_minutes for it in items)
            plan = PlanModel(
                items=items,
                total_duration_minutes=total_duration,
                total_travel_time_minutes=sum(it.travel_time_minutes or 0 for it in items),
                summary="Автоплан без LLM",
                included_events=[ev.title for ev in events if ev.title],
                excluded_events=[]
            )
            print("✅ План создан детерминированно (без LLM)")
            return plan
        ...
        try:
            ...
            if plan is None:
                print("⚠️ LLM вернул None вместо плана")
                from src.planner_agent.models import Plan as PlanModel
                plan = PlanModel(
                    items=[],
                    total_duration_minutes=0,
                    total_travel_time_minutes=0,
                    summary="План не был создан (LLM вернул None)",
                    included_events=[],
                    excluded_events=[]
                )
            
            print("✅ План создан:")
            print(f"   - Событий в плане: {len(plan.items) if plan.items else 0}")
            ...
            print(f"   - Включено событий: {len(plan.included_events) if plan.included_events else 0}")
            ...
            print(f"❌ Ошибка при создании плана в create_plan: {error_msg}")
            print(f"Детали: {error_trace[:500]}")
            
            # Всегда возвращаем объект Plan, даже при ошибке
            from src.planner_agent.models import Plan as PlanModel
            error_plan = PlanModel(
                items=[],
                total_duration_minutes=0,
                total_travel_time_minutes=0,
                summary=f"Ошибка при создании плана: {error_msg[:100]}",
                included_events=[],
                excluded_events=[]
            )
            return error_plan
...
Дай конструктивную критику с конкретными предложениями по улучшению."""
        ...
        try:
            ...
            critique = Critique(
                overall_assessment="Не удалось получить критику",
                strengths=[],
                weaknesses=[],
                suggestions=[],
                critical_issues=[],
        ...
        if critique:
            ...
            print(f"   - Сильных сторон: {len(critique.strengths) if critique.strengths else 0}")
            print(f"   - Слабых сторон: {len(critique.weaknesses) if critique.weaknesses else 0}")
            print(f"   - Предложений: {len(critique.suggestions) if critique.suggestions else 0}")
            print(f"   - Критических проблем: {len(critique.critical_issues) if critique.critical_issues else 0}")
            print(f"   - Требуется пересмотр: {'Да' if critique.needs_revision else 'Нет'}")
        else:
            print("⚠️ Критика не получена")
            # Создаем минимальную критику
            critique = Critique(
