...
- **Python 3.8+** (только для создания Telegram session)
...
- ✅ Проверит все необходимые токены
- ✅ Спросит о парсинге Telegram каналов
- ✅ При необходимости создаст Telegram session
...
# 4. (Опционально) Создайте Telegram session для парсинга каналов
...
# Проверить данные в Weaviate
curl http://localhost:8000/weaviate/stats
```

### Что нужно для запуска?

| Токен | Обязательный | Для чего | Где получить |
|-------|:------------:|----------|--------------|
| `TELEGRAM_BOT_TOKEN` | ✅ Да | Telegram бот | [@BotFather](https://t.me/BotFather) |
| `OPENAI_API_KEY` | ✅ Да | LLM для планирования | [OpenAI](https://platform.openai.com/api-keys) |
| `TELEGRAM_APP_API_ID` | ❌ Нет | Парсинг каналов | [my.telegram.org](https://my.telegram.org/apps) |
| `TELEGRAM_APP_API_HASH` | ❌ Нет | Парсинг каналов | [my.telegram.org](https://my.telegram.org/apps) |
| `OPENWEATHER_API_KEY` | ❌ Нет | Погода в планах | [OpenWeather](https://openweathermap.org/api) |
| `YANDEX_GEOCODER_API_KEY` | ❌ Нет | Карты и маршруты | [Yandex](https://developer.tech.yandex.ru/) |
| `TAVILY_API_KEY` | ❌ Нет | Веб-поиск | [Tavily](https://tavily.com/) |

> 💡 **Минимальный набор**: Для работы достаточно `TELEGRAM_BOT_TOKEN` и `OPENAI_API_KEY`. Остальные токены расширяют функциональность.

---

## 📖 Подробная инструкция по запуску

### Вариант 1: Автоматический (launch.sh)

Скрипт `launch.sh` — самый простой способ запустить проект:
...
1. **Проверяет Docker** — убеждается что Docker установлен и запущен
2. **Создаёт .env** — если файла нет, создаёт шаблон и просит заполнить
3. **Проверяет токены** — валидирует обязательные переменные
4. **Спрашивает о Telegram** — предлагает настроить парсинг каналов
5. **Создаёт session** — если нужно, запускает `get_telegram_session.sh`
6. **Собирает образы** — запускает `docker compose build`
7. **Запускает сервисы** — запускает `docker compose up -d`
...
|--------|----------|------|
| **weaviate** | Векторная БД | 8080 |
| **contextionary** | Модуль векторизации | — |
| **api** | REST API для управления | 8000 |
| **bot** | Telegram бот | — |
| **sync-worker** | Фоновая синхронизация каналов | — |

#### Запуск без парсинга Telegram (без sync-worker)
...
# Терминал 2: Sync Worker (опционально)
...
### Вариант 4: Использование main_pipeline напрямую
