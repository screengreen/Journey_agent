    ...
    def retrieve(
        ...
        try:
            ...
            for obj in result.objects:
                try:
                    # Фильтруем по тегам программно
                    obj_tags = obj.properties.get("tags", [])
                    if user_tag:
                        # Проверяем, содержит ли объект нужный тег или тег 'all'
                        if user_tag not in obj_tags and "all" not in obj_tags:
                            continue
