...
class Event(BaseModel):
    """Событие с его настройками."""
    name: str = Field(description="Название события")
    address: str = Field(description="Адрес события")
    duration_minutes: int = Field(description="Продолжительность события в минутах")
    priority: int = Field(description="Приоритет события (1-10)", ge=1, le=10)
    required: bool = Field(description="Обязательно ли событие", default=False)
    metadata: Dict[str, Any] = Field(description="Дополнительные данные", default_factory=dict)
