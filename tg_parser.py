...
if __name__ == "__main__":
    async def example():
        ...
        async with TelegramParser() as parser:
            ...
            print(f"\nПоследние {len(messages)} сообщений из канала:\n")
            parser.print_messages(messages)
            
            # Получение сообщений из избранного
            saved_messages = await parser.get_saved_messages(limit=5, text_only=True, reverse=True)
            print(f"\nПоследние {len(saved_messages)} сообщений из избранного:\n")
            parser.print_messages(saved_messages)
    
    # Запуск примера
    TelegramParser.run_async(example())
